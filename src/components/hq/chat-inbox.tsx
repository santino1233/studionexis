...
type Thread = { id: string; tenantId: string; studio: string; slug: string; plan: string; contact: string | null; status: string; messages: Msg[] };
...
export function HqChatInbox() {
  ...
  const post = async (t: string, image?: string) => {
    if (!sel) return;
    setThread((th) => th ? { ...th, messages: [...th.messages, { from: "studio", name: "Nexis Support", text: t, at: new Date().toISOString(), ...(image ? { image } : {}) }] } : th);
    await fetch("/api/hq/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: sel, text: t, ...(image ? { image } : {}) }) }).catch(() => {});
    pull();
  };
  const send = async () => {
    const t = text.trim();
  ...
  const attach = async (file: File) => {
    if (!file || !sel || !thread || busy) return;
    setBusy(true);
    try {
      const fd = new FormData();
      fd.append("file", file);
      fd.append("tenantId", thread.tenantId);
